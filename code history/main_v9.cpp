...
SemaphoreHandle_t keyPressMutex;
...
void sampleISR()
{
    ...
    for(uint8_t i=0;i<keypressed_pointer_local;i++){
        // xSemaphoreTake(keyPressMutex, portMAX_DELAY); 
        keypressed_local[i] = keypressed[i];
        // xSemaphoreGive(keyPressMutex);
    }
    ...
    while (1)
    {
        ...
        else
        {
            ...
            for (uint8_t i = 0; i < 12; i++)
            {
                ...
                {
                    __atomic_store_n(&keypressed[num_keypressed], i, __ATOMIC_RELAXED);
                    num_keypressed += 1;
                }
            }
        ...
        if (num_keypressed > 0)
        {
            for (uint8_t i = 0; i < num_keypressed; i++)
            {
                ...
                if (octave_local < 4)
                {
                    __atomic_store_n(&currentStepSize[i], stepSizes[keypressed[i]] >> (4 - octave_local), __ATOMIC_RELAXED);
                }
                else
                {
                    __atomic_store_n(&currentStepSize[i], stepSizes[keypressed[i]] << (octave_local - 4), __ATOMIC_RELAXED);
                }
            }
        }
...
void setup()
{
    ...
    keyPressMutex = xSemaphoreCreateMutex();
